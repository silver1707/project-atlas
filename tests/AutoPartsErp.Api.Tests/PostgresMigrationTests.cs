...
public sealed class PostgresMigrationTests : IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder()
        ...
        .WithDatabase("autoparts_erp_tests")
        .WithUsername("atlas")
        .WithPassword("autoparts_test_password")
        .Build();
    ...
    public async Task Initial_migration_creates_rls_partitioned_tables_and_seed_data()
    {
        ...
        var migration = await File.ReadAllTextAsync(Path.Combine(root, "database", "migrations", "001_initial.sql"));
        var seed = await File.ReadAllTextAsync(Path.Combine(root, "database", "migrations", "002_seed.sql"));
